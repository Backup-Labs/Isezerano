...
export default function NewsroomLayout({
  ...
  if (loading || !user || user.role === 'reader') {
    return (
      <div className="min-h-screen bg-theme-white text-theme-black flex flex-col items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-theme-white text-theme-black flex flex-col md:flex-row">
      {/* Sidebar Frame */}
      <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r border-theme-gray-100 bg-theme-light-gray flex flex-col justify-between shrink-0 p-6">
        <div className="flex flex-col gap-8">
          ...
          <div className="flex items-center gap-2 pb-4 border-b border-theme-gray-100">
            ...
            <span className="serif-title text-xl font-bold tracking-tight text-theme-black uppercase">
              NEWSROOM
            </span>
          </div>

          {/* User details summary */}
          <div className="border border-theme-gray-100 p-4 flex flex-col gap-1 bg-theme-white">
            <div className="text-xs font-bold text-theme-black font-mono uppercase">{user.first_name || user.username}</div>
          ...
          <nav className="flex flex-col gap-1.5">
            {allowedLinks.map((link) => (
              <Link 
                key={link.name} 
                href={link.href}
                className="flex items-center gap-3 px-4 py-2.5 text-xs font-mono font-bold uppercase tracking-wider text-theme-gray-400 hover:text-theme-blue hover:bg-theme-light-gray transition-all border border-transparent hover:border-theme-gray-100"
              >
                {link.icon}
                <span>{link.name}</span>
              </Link>
